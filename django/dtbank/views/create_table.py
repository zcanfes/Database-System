    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("""
            CREATE TABLE Interacts ( -- Drug interactions with other drugs
                drugbank_id1 CHAR(7), -- drug 1
                drugbank_id2 CHAR(7), -- drug that interacts with drug 1
        ...
        cursor.execute("""
            CREATE TABLE Target_protein ( -- Drug's target protein
                uniprot_id CHAR(6), -- id of the protein
        ...
        cursor.execute("""
            CREATE TABLE Side_effects ( -- Side effects and their id's
                umlscui CHAR(8), -- max length of the umlscui is 8 in the table
        ...
        cursor.execute("""
            CREATE TABLE Article ( -- Article information
                ...
                username VARCHAR, -- username first author of the article
                institute_name VARCHAR, -- institute of the first author
                PRIMARY KEY (DOI),
                FOREIGN KEY (username, institute_name) REFERENCES DT_User
        ...
        cursor.execute("""
            CREATE TABLE Has_authors ( -- Article and authors relation
                DOI VARCHAR, -- DOI of the article
                author_name VARCHAR, -- one author of the article
                PRIMARY KEY (author_name, DOI),
                FOREIGN KEY (DOI) REFERENCES Article -- If delete the article, delete the authors containing it
                    ON DELETE CASCADE -- Delete this relation when the article is updated/deleted.
                    ON UPDATE CASCADE
            );
        """)
